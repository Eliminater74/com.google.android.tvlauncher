...
final class ActiveResources {
    ...
    public void cleanReferenceQueue() {
        while (!this.isShutdown) {
            try {
                ...
                DequeuedResourceCallback current = this.f49cb;
                if (current != null) {
    ...
    static final class ResourceWeakReference extends WeakReference<EngineResource<?>> {
        ...
        ResourceWeakReference(@NonNull Key key2, @NonNull EngineResource<?> referent, @NonNull ReferenceQueue<? super EngineResource<?>> queue, boolean isActiveResourceRetentionAllowed) {
            ...
            this.resource = (!referent.isMemoryCacheable() || !isActiveResourceRetentionAllowed) ? null : (Resource) Preconditions.checkNotNull(referent.getResource());
