...
public final class Collections2 {
    ...
    private static class PermutationIterator<E> extends AbstractIterator<List<E>> {
        ...
        PermutationIterator(List<E> list2) {
            ...
            int n = list2.size();
            this.f167c = new int[n];
            this.f169o = new int[n];
            Arrays.fill(this.f167c, 0);
            Arrays.fill(this.f169o, 1);
        }
        ...
        /* access modifiers changed from: package-private */
        public void calculateNextPermutation() {
            this.f168j = this.list.size() - 1;
            int s = 0;
            if (this.f168j != -1) {
                while (true) {
                    int[] iArr = this.f167c;
                    int i = this.f168j;
                    int q = iArr[i] + this.f169o[i];
                    if (q < 0) {
                        switchDirection();
                    } else if (q != i + 1) {
                        Collections.swap(this.list, (i - iArr[i]) + s, (i - q) + s);
                        this.f167c[this.f168j] = q;
                        return;
                    } else if (i != 0) {
                        s++;
                        switchDirection();
