...
public class Hex {
    ...
    public static String bytesToStringUppercase(byte[] bArr, boolean z) {
        ...
        while (i < length && (!z || i != length - 1 || (bArr[i] & UnsignedBytes.MAX_VALUE) != 0)) {
            ...
            sb.append(zza[bArr[i] & Ascii.f156SI]);
            i++;
        }
        return sb.toString();
    }
    ...
    public static String bytesToColonDelimitedStringUppercase(byte[] bArr) {
        ...
        while (i < bArr.length - 1) {
            ...
            cArr[i2] = cArr2[b >>> 4];
            int i4 = i3 + 1;
            cArr[i3] = cArr2[b & Ascii.f156SI];
            cArr[i4] = ':';
            i++;
            i2 = i4 + 1;
        }
        byte b2 = bArr[bArr.length - 1] & UnsignedBytes.MAX_VALUE;
        char[] cArr3 = zza;
        cArr[i2] = cArr3[b2 >>> 4];
        cArr[i2 + 1] = cArr3[b2 & Ascii.f156SI];
        return new String(cArr);
    }
